...
export default function DgwOwnershipPage() {
  ...
  // Find DGW teams — teams with 2+ fixtures in an upcoming GW
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">DGW Ownership</h1>
        <p className="text-gray-400 text-sm mt-1">
          Players from teams with double gameweeks and their ownership levels.
        </p>
      </div>

      {dgwInfo.dgwGw ? (
        <>
          <div className="bg-fpl-card border border-fpl-accent/30 rounded-lg p-4">
            <p className="text-fpl-accent font-semibold">
              Next Double Gameweek: GW {dgwInfo.dgwGw}
            </p>
            <p className="text-sm text-gray-400 mt-1">
              Teams with 2+ fixtures: {dgwInfo.dgwTeams.map((id) => teamsMap?.[id]?.short_name).filter(Boolean).join(', ')}
            </p>
          </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {dgwInfo.gwFixtures?.map((f) => {
              ...
              return (
                <div key={f.id} className="bg-fpl-card border border-fpl-border rounded-lg p-3 flex items-center justify-between">
                  <span className={`font-medium ${isDgwHome ? 'text-fpl-accent' : 'text-white'}`}>{home?.short_name}</span>
                  <div className="flex gap-1">
                    <DifficultyBadge difficulty={f.team_h_difficulty} />
                    <span className="text-gray-500">vs</span>
                    <DifficultyBadge difficulty={f.team_a_difficulty} />
                  </div>
                  <span className={`font-medium ${isDgwAway ? 'text-fpl-accent' : 'text-white'}`}>{away?.short_name}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-fpl-card border border-fpl-border rounded-lg p-8 text-center">
          <p className="text-gray-400">No upcoming double gameweeks detected.</p>
          <p className="text-sm text-gray-500 mt-1">DGW fixtures will appear here when announced by the Premier League.</p>
        </div>
      )}
    </div>
  );
