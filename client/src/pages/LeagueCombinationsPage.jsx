...
export default function LeagueCombinationsPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">{t('leagueCombinations.title')}</h1>
      <p className="text-gray-500 dark:text-gray-400 text-sm">{t('leagueCombinations.subtitle')}</p>
      ...
      {league && combinations.length > 0 && (
        <div className="bg-white dark:bg-fpl-card border border-fpl-light-border dark:border-fpl-border rounded-lg overflow-hidden">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white px-4 py-3 border-b border-fpl-light-border dark:border-fpl-border">{league.name}</h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-gray-500 dark:text-gray-400 border-b border-fpl-light-border dark:border-fpl-border">
                  ...
                  <th className="px-4 py-2 text-right">{t('leagueCombinations.gapAbove')}</th>
                  <th className="px-4 py-2 text-right">{t('leagueCombinations.gapToFirst')}</th>
              ...
              <tbody>
                {combinations.map((s) => (
                  <tr key={s.entry} className="border-b border-fpl-light-border/30 dark:border-fpl-border/30">
                    ...
                    <td className="px-4 py-2 text-gray-900 dark:text-white font-medium">{s.entry_name}</td>
                    <td className="px-4 py-2 text-right text-gray-900 dark:text-white">{s.total}</td>
                    ...
                    <td className={`px-4 py-2 text-right font-medium ${s.gapToFirst > 0 ? 'text-fpl-red' : 'text-fpl-green'}`}>
                      {s.gapToFirst > 0 ? `-${s.gapToFirst}` : t('leagueCombinations.leader')}
                    </td>
                    <td className="px-4 py-2 text-right text-gray-600 dark:text-gray-300">{s.event_total}</td>
                  </tr>
                ))}
              </tbody>
            </table>
